...
// ------------------------------- Encoding: primitives -------------------------------
    ...
    type private SolverBuilder<'IExpr, 'IBoolExpr, 'IBitVecExpr, 'IFPExpr, 'IFuncDecl, 'ISort, 'IModel, 'ISolver, 'IParams>(ctx : ISolverCommon<'IExpr, 'IBoolExpr, 'IBitVecExpr, 'IFPExpr, 'IFuncDecl,
...
// ------------------------------- Encoding: logic simplifications -------------------------------
        ...
        member private x.MkNot (expr : 'IBoolExpr) =
            match expr with
            | True -> FalseExpr
            | False -> TrueExpr
            | _ -> ctx.MkNot expr
        ...
        member private x.SimplifyAndElements (elems : 'IBoolExpr seq) =
            Seq.filter (fun elem -> elem <> (TrueExpr :?> 'IBoolExpr)) elems |> Seq.distinct
        ...
        member x.MkAnd (elems : 'IBoolExpr seq) =
        ...
        member private x.MkOr (elems : 'IBoolExpr seq) =
        ...
        member private x.MkITE(cond : 'IBoolExpr, thenExpr, elseExpr) : 'IExpr =
            ...
            | _, (:? 'IBitVecExpr as thenExpr), (:? 'IBitVecExpr as elseExpr) ->
...
// ------------------------------- Encoding: arithmetic simplifications -------------------------------
        ...
        member private x.ExtendIfNeed (x : 'IExpr, y : 'IExpr as args) isSigned =
        ...
        member private this.ChangeSizeIfNeed (x : 'IBitVecExpr, y : 'IBitVecExpr as args) typ =
        ...
        member private x.MkEq(left : 'IExpr, right : 'IExpr) =
            ...
            | :? 'IBitVecExpr as l, (:? 'IBitVecExpr as r) ->
                x.ExtendIfNeed(l, r) true |> ctx.MkEq
            | _ -> ctx.MkEq(left , right)

        member private x.MkBVSGT(left, right as operands) : 'IBoolExpr =
            match left, right with
            | _ when left = right -> FalseExpr
            | _ -> x.ExtendIfNeed operands true |> ctx.MkBVSGT

        member private x.MkBVUGT(left, right as operands) : 'IBoolExpr =
            match left, right with
            | _ when left = right -> FalseExpr
            | _ -> x.ExtendIfNeed operands false |> ctx.MkBVUGT

        member private x.MkBVSGE(left, right as operands) : 'IBoolExpr =
            match left, right with
            | _ when left = right -> TrueExpr
            | _ -> x.ExtendIfNeed operands true |> ctx.MkBVSGEq

        member private x.MkBVUGE(left, right as operands) : 'IBoolExpr =
            match left, right with
            | _ when left = right -> TrueExpr
            | _ -> x.ExtendIfNeed operands false |> ctx.MkBVUGEq

        member private x.MkBVSLT(left, right as operands) : 'IBoolExpr =
            match left, right with
            | _ when left = right -> FalseExpr
            | _ -> x.ExtendIfNeed operands true |> ctx.MkBVSLT

        member private x.MkBVULT(left, right as operands) : 'IBoolExpr =
            match left, right with
            | _ when left = right -> FalseExpr
        ...
        member private x.MkBVULE(left, right as operands) : 'IBoolExpr =
            match left, right with
            | _ when left = right -> TrueExpr
            | _ -> x.ExtendIfNeed operands false |> ctx.MkBVULEq

        member private x.MkBVAndT typ operands : 'IBitVecExpr =
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVAnd

        member private x.MkBVOrT typ operands : 'IBitVecExpr =
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVOr

        member private x.MkBVOr operands : 'IBitVecExpr =
            x.ExtendIfNeed operands false |> ctx.MkBVOr

        member private x.MkBVXorT typ operands : 'IBitVecExpr =
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVXor

        member private x.MkBVShlT typ operands : 'IBitVecExpr =
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVSHL

        member private x.MkBVShl operands : 'IBitVecExpr =
            x.ExtendIfNeed operands true |> ctx.MkBVSHL

        member private x.MkBVAShrT typ operands : 'IBitVecExpr =
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVASHR

        member private x.MkBVLShrT typ operands : 'IBitVecExpr =
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVLSHR

        member private x.MkBVLShr operands : 'IBitVecExpr =
            x.ExtendIfNeed operands true |> ctx.MkBVLSHR

        member private x.MkBVAddT typ operands :'IBitVecExpr =
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVAdd

        member private x.MkBVAdd operands : 'IBitVecExpr =
        ...
        member private x.MkBVMulT typ operands : 'IBitVecExpr =
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVMul

        member private x.MkBVMul operands : 'IBitVecExpr =
        ...
        member private x.MkBVSubT typ operands : 'IBitVecExpr =
        ...
        member private x.MkBVSDivT typ operands : 'IBitVecExpr =
            assert(isSigned typ)
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVSDiv

        member private x.MkBVUDivT typ operands : 'IBitVecExpr =
            assert(isUnsigned typ)
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVUDiv

        member private x.MkBVSRemT typ operands : 'IBitVecExpr =
            assert(isSigned typ)
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVSRem

        member private x.MkBVURemT typ operands : 'IBitVecExpr =
            assert(isUnsigned typ)
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVURem

        member private x.Max (left : 'IBitVecExpr) (right : 'IBitVecExpr) =
            assert(ctx.SortSize left = ctx.SortSize right)
            x.MkITE(x.MkBVSGT(left, right), left, right) :?> 'IBitVecExpr

        member private x.Min (left : 'IBitVecExpr) (right : 'IBitVecExpr) =
            assert(ctx.SortSize left = ctx.SortSize right)
            x.MkITE(x.MkBVSGT(left, right), right, left) :?> 'IBitVecExpr
...
// ------------------------------- Floating point arithmetic simplifications -------------------------------
        ...
        member private x.MkFPAdd(left : 'IFPExpr, right : 'IFPExpr as operands) : 'IFPExpr =
            assert (ctx.GetSort left = ctx.GetSort right)
            let roundNearest = ctx.MkFPRoundingMode()
...
// ------------------------------- Encoding: common -------------------------------
        ...
        member private x.EncodeConcrete encCtx (obj : obj) typ : encodingResult =
            let expr =
                ...
                | Numeric t when t = typeof<float> -> ctx.MkFPDoubleNumeral(obj :?> float, (x.Type2Sort typ) :?> FPSort)
...
// ------------------------------- Encoding: expression -------------------------------
        ...
        member private x.ComputeSliceBounds assumptions cuts termSortSize =
            ...
            let zero = ctx.MkBVInt(0, termSortSize)
            ...
            let addBounds (startByte, endByte, pos) (assumptions, startExpr, sizeExpr, position) =
                ...
                let startByte = x.ExtractOrExtend (startByte.expr :?> 'IBitVecExpr) termSortSize
                let endByte = x.ExtractOrExtend (endByte.expr :?> 'IBitVecExpr) termSortSize
                let pos = x.ExtractOrExtend (pos.expr :?> 'IBitVecExpr) termSortSize
        ...
        member private x.EncodeCombine encCtx slices typ =
            let res = ctx.MkNumeral(0, x.Type2Sort typ) :?> 'IBitVecExpr
            let window = ctx.SortSize res
            let windowExpr = ctx.MkNumeral(window, x.Type2Sort(Types.IndexType)) :?> 'IBitVecExpr
            let addOneSlice (res, assumptions) slice =
                ...
                let t =
                    if term.expr :? 'IFPExpr then ctx.MkFPToIEEEBV (term.expr :?> 'IFPExpr) :?> 'IBitVecExpr
                    else term.expr :?> 'IBitVecExpr
                ...
                let lBit = x.MkBVMul(lByte, ctx.MkBVInt(8, termSize))
                let rBit = x.MkBVMul(rByte, ctx.MkBVInt(8, termSize))
                let posBit = x.MkBVMul(posByte, ctx.MkBVInt(8, termSize))
                let sliceSize = x.MkBVSub(rBit, lBit)
                let zero = ctx.MkBVInt(0, termSize)
                ...
                let res = x.MkITE(intersects, x.MkBVOr(res, part), res) :?> 'IBitVecExpr
                res, assumptions
        ...
        member private x.EncodeExpression encCtx term op args typ =
            encodingCache.Get(term, fun () ->
                ...
                | Cast(Numeric t1, Numeric t2) when isReal t1 && isReal t2 ->
                    ...
                    let newExpr = ctx.MkFPToFP(roundNearest, expr.expr :?> 'IFPExpr, sort)
                    {expr = newExpr; assumptions = expr.assumptions}
                | Cast(Numeric t1, Numeric t2) when isReal t1 ->
                    let roundNearest = ctx.MkFPRoundingMode()
                    let expr = x.EncodeTerm encCtx (List.head args)
                    let size = numericSizeOf t2
                    let sign = isUnsigned t2
                    {expr = ctx.MkFPToBV(roundNearest, expr.expr :?> 'IFPExpr, size, sign); assumptions = expr.assumptions}
                | Cast(Numeric t1, Numeric t2) when isReal t2 ->
                    let roundNearest = ctx.MkFPRoundingMode()
                    let expr = x.EncodeTerm encCtx (List.head args)
                    let sort = x.Type2Sort t2
                    let sign = isUnsigned t1
                    let newExpr = ctx.MkFPToFP(roundNearest, expr.expr :?> 'IBitVecExpr, sort, sign)
                    {expr = newExpr; assumptions = expr.assumptions}
                | Cast(Numeric t1, Numeric t2) when isLessForNumericTypes t1 t2 ->
                    ...
                    {expr = extend(difference, expr.expr :?> 'IBitVecExpr); assumptions = expr.assumptions}
                | Cast(Numeric t1, Numeric t2) when isLessForNumericTypes t2 t1 ->
                    let expr = x.EncodeTerm encCtx (List.head args)
                    let from = numericBitSizeOf t2 - 1u
                    {expr = ctx.MkExtract(from, 0u, expr.expr :?> 'IBitVecExpr); assumptions = expr.assumptions}
        ...
        member private this.MakeBinary<'a, 'b, 'c when 'a :> 'IExpr and 'a : equality and 'b :> 'IExpr and 'b : equality and 'c :> 'IExpr and 'c : equality>
                (encCtx : encodingContext)
                (constructor : 'a * 'b -> 'c)
                (args : term list) : encodingResult =
            match args with
...
// ------------------------------- Encoding: memory reading -------------------------------
        ...
        member private x.KeyInVectorTimeIntervals encCtx (key : 'IExpr) acc (region : vectorTime intervals) =
            let onePointCondition acc (y : vectorTime endpoint) =
                let bound = ctx.MkNumeral(encCtx.addressOrder[y.elem], x.Type2Sort addressType) :?> 'IBitVecExpr
                let condition =
                    match y.sort with
                    | endpointSort.OpenRight -> x.MkBVSLT(key :?> 'IBitVecExpr, bound)
                    | endpointSort.ClosedRight -> x.MkBVSLE(key :?> 'IBitVecExpr, bound)
                    | endpointSort.OpenLeft -> x.MkBVSGT(key :?> 'IBitVecExpr, bound)
                    | endpointSort.ClosedLeft -> x.MkBVSGE(key :?> 'IBitVecExpr, bound)
                    | _ -> __unreachable__()
        ...
        member private x.HeapAddressKeyInRegion encCtx acc (keyExpr : 'IExpr) region =
        ...
        member private x.ArrayOneIndexKeyInRegion encCtx acc (keyExpr : 'IExpr[]) region =
        ...
        member private x.VectorIndexKeyInRegion encCtx acc (keyExpr : 'IExpr[]) region =
        ...
        member private x.GetRegionConstant (name : string) sort (path : path) (regSort : regionSort) =
            let mkConst () = ctx.MkConst (name, sort) :?> 'IArrayExpr
        ...
        member private x.GenerateInstAssumptions (inst : 'IExpr) typ =
            if typ = addressType then
                ...
                let belowZero = x.MkBVSLE(inst :?> 'IBitVecExpr, zero)
        ...
        member private x.GenerateLengthAssumptions encodingResult =
            let expr = encodingResult.expr :?> 'IBitVecExpr
            let assumptions = encodingResult.assumptions
            let lengthIsNonNegative = x.MkBVSGE(expr, ctx.MkBVInt(0, ctx.SortSize expr))
            let assumptions = lengthIsNonNegative :: assumptions
            let assumptions =
                if maxBufferSize < 0 then assumptions
                else x.MkBVSLE(expr, ctx.MkBVInt(maxBufferSize, ctx.SortSize expr)) :: assumptions
        ...
        member private x.GenerateCharAssumptions encodingResult =
            ...
            let expr = encodingResult.expr :?> 'IBitVecExpr
            let assumptions = encodingResult.assumptions
            let left = ctx.MkBVSGT(expr, ctx.MkBVInt(32, ctx.SortSize expr))
            let right = ctx.MkBVSLT(expr, ctx.MkBVInt(127, ctx.SortSize expr))
        ...
        member private x.ArrayReading encCtx specialize keyInRegion keysAreMatch encodeKey hasDefaultValue indices key mo typ source path name =
            ...
            let inst (k : Expr[]) =
                if hasDefaultValue then x.DefaultValue valueSort, List.empty
                    ...
                    let expr = ctx.MkRangeSelect(array :?> 'IExpr, k)
                    expr, x.GenerateInstAssumptions expr typ
            let res = x.MemoryReading encCtx specialize keyInRegion keysAreMatch encodeKey inst path key mo
        ...
        member private x.ArrayRangeKeyMatches encCtx leftExpr address fromIndices toIndices rightRegion =
            ...
            let keyInBounds (i : Expr) (l : Expr) (r : Expr) =
                let i = i :?> 'IBitVecExpr
                x.MkAnd(x.MkBVSLE(l :?> 'IBitVecExpr, i), x.MkBVSLE(i, r :?> 'IBitVecExpr))
        ...
        member private x.StructReading encCtx (structSource : ISymbolicConstantSource) (field : fieldId) typ (path : path) name =
            ...
            | _ when field.declaringType = typeof<decimal> && field.name = "_flags" ->
                let expr = res.expr :?> 'IBitVecExpr
                let lowerWord = ctx.MkExtract(15u, 0u, expr)
                let lowerIsZero = x.MkEq(lowerWord, ctx.MkBVInt(0, ctx.SortSize lowerWord))
                let exp = ctx.MkExtract(23u, 16u, expr)
                let expSize = ctx.SortSize exp
                let leftBound = x.MkBVUGE(exp, ctx.MkBVInt(0, expSize))
                let rightBound = x.MkBVULE(exp, ctx.MkBVInt(28, expSize))
                let expInBound = x.MkAnd(leftBound, rightBound)
                let upper = ctx.MkExtract(30u, 24u, expr)
                let upperIsZero = x.MkEq(upper, ctx.MkBVInt(0, ctx.SortSize upper))
    ...
    // ------------------------------- Decoding -------------------------------
        ...
        member private x.DecodeFp t (fp : 'IExpr) =
    ...
    let private ctxZ3 = new Context()
    let private ctx = new Z3Solver(ctxZ3)
    let private builder = SolverBuilder(ctx :> ISolverCommon<'IExpr, 'IBoolExpr, 'IBitVecExpr, 'IFPExpr, 'IFuncDecl, 'ISort, 'IModel, 'ISolver, 'IParams>)

    type internal Z3Solver(timeoutMs : uint option, ctx : ) =
