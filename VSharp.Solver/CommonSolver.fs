...
// ------------------------------- Cache -------------------------------
    ...
    type private encodingCache<'IExpr, 'IBoolExpr, 'IArrayExpr, 'ISort, 'IFuncDecl when 'IExpr : equality and 'ISort : equality> = {
        sorts : IDictionary<Type, 'ISort>
        e2t : IDictionary<'IExpr, term>
        t2e : IDictionary<term, encodingResult<'IExpr, 'IBoolExpr>>
        heapAddresses : IDictionary<'IExpr, vectorTime>
        staticKeys : IDictionary<'IExpr, Type>
        regionConstants : Dictionary<regionSort * path, 'IArrayExpr>
        defaultValues : IDictionary<regionSort, HashSet<'IExpr[]>>
        funcDecls : IDictionary<string * 'ISort[] * 'ISort, 'IFuncDecl>
        mutable lastSymbolicAddress : int32
    } with
        member x.Get(term, encoder : unit -> 'IExpr) =
            Dict.tryGetValue2 x.t2e term (fun () ->
                let result = {expr = encoder(); assumptions = List.empty}
                x.e2t[result.expr] <- term
                x.t2e[term] <- result
                result)
        member x.Get(term, encoder : unit -> encodingResult<'IExpr,'IBoolExpr>) =
            Dict.tryGetValue2 x.t2e term (fun () ->
                let result = encoder()
                x.e2t[result.expr] <- term
                x.t2e[term] <- result
    ...
    let private freshCache<'IExpr, 'IBoolExpr, 'IArrayExpr, 'ISort, 'IFuncDecl when 'IExpr : equality and 'ISort : equality> () = {
        sorts = Dictionary<Type, 'ISort>()
        e2t = Dictionary<'IExpr, term>()
        t2e = Dictionary<term, encodingResult<'IExpr, 'IBoolExpr>>()
        heapAddresses = Dictionary<'IExpr, vectorTime>()
        staticKeys = Dictionary<'IExpr, Type>()
        regionConstants = Dictionary<regionSort * path, 'IArrayExpr>()
        defaultValues = Dictionary<regionSort, HashSet<'IExpr[]>>()
        funcDecls = Dictionary<string * 'ISort[] * 'ISort, 'IFuncDecl>()
...
// ------------------------------- Encoding: primitives -------------------------------
    ...
    type internal SolverBuilder<'IExpr, 'IBoolExpr, 'IBitVecExpr, 'IFPExpr, 'IArrayExpr, 'IBitVecNum, 'IFPNum, 'IFuncDecl,
        ...
        let mutable encodingCache = freshCache<'IExpr, 'IBoolExpr, 'IArrayExpr, 'ISort, 'IFuncDecl>()
        let emptyState = Memory.EmptyState()
        let mutable maxBufferSize = 128

        let getMemoryConstant mkConst (typ : regionSort * path) =
            let result : 'IArrayExpr ref = ref null
        ...
        let getDefaultValues (regionSort : regionSort) =
            let result : HashSet<'IExpr[]> ref = ref null
            if encodingCache.defaultValues.TryGetValue(regionSort, result) then result.Value
            else
                let suitableKeys = HashSet<'IExpr[]>()
                encodingCache.defaultValues.Add(regionSort, suitableKeys)
                suitableKeys

        let getFuncDecl (name : string) (argsSort : 'ISort[]) (resultSort : 'ISort) =
            let result : 'IFuncDecl ref = ref null
        ...
        let (|True|_|) (expr : 'IBoolExpr) =
            match expr with
            | _ when expr = TrueExpr -> Some(True)
            | _ -> None

        let (|False|_|) (expr : 'IBoolExpr) =
            match expr with
        ...
        member x.MkEToBE(expr : 'IExpr) = ctx.MkEToBE expr
        member x.MkSAssert(s: 'ISolver, [<ParamArray>] cond : 'IBoolExpr array) = ctx.Assert(s, cond)
        member x.Reset() =
            encodingCache <- freshCache<'IExpr, 'IBoolExpr, 'IArrayExpr, 'ISort, 'IFuncDecl>()
        ...
        member private x.AddressSort = ctx.MkBitVecSort(32u)
        ...
        member private x.Type2Sort typ =
            Dict.getValueOrUpdate encodingCache.sorts typ (fun () ->
                match typ with
                | Bool -> ctx.MkBoolSort()
                | typ when typ.IsEnum -> ctx.MkBitVecSort(numericSizeOf typ)
                | typ when isIntegral typ -> ctx.MkBitVecSort(numericSizeOf typ)
                | typ when typ = typeof<single> -> ctx.MkFPSort32()
                | typ when typ = typeof<double> -> ctx.MkFPSort64()
        ...
        member private x.DefaultValue (sort : 'ISort) =
            if sort = ctx.BoolSort() then ctx.MkFalse() |> ctx.MkBEToE
        ...
        member private x.CreateConstant name typ =
            ...
            let assumptions =
                if typ = addressType then
                    ...
                    ctx.MkBVSLEq(constant, zero) |> List.singleton
                else List.empty
            {expr = constant; assumptions = assumptions}

// ------------------------------- Encoding: logic simplifications -------------------------------

        member private x.MkNot (expr : 'IExpr) =
            let expr = ctx.MkEToBE expr
            match expr with
            | True -> FalseExpr
            | False -> TrueExpr
            | _ -> ctx.MkNot expr

        member private x.MkAnd(left : 'IBoolExpr, right : 'IBoolExpr) =
        ...
        member private x.MkOr(left : 'IBoolExpr, right : 'IBoolExpr) =
            match left, right with
            | _ when left = right -> left
            | True, _ -> left
            | _, True -> right
        ...
        member private x.SimplifyAndElements (elems : 'IBoolExpr seq) =
            Seq.filter (fun elem -> elem <> TrueExpr ) elems |> Seq.distinct

        member private x.CreateAnd (elems : 'IBoolExpr seq) =
            match elems with
            | Seq.Empty -> TrueExpr
            | Seq.Cons(head, tail) when Seq.isEmpty tail -> head
            | elems -> ctx.MkAndSeq(elems)

        member x.MkAnd ([<ParamArray>] elems : 'IBoolExpr[]) =
            if Array.contains FalseExpr elems then FalseExpr
            else x.SimplifyAndElements elems |> x.CreateAnd

        member x.MkAnd (elems : 'IExpr seq) =
            let elems = elems |> Seq.map (fun m -> ctx.MkEToBE m) |> Seq.toList
            if Seq.contains FalseExpr elems then FalseExpr
            else x.SimplifyAndElements elems |> x.CreateAnd

        member private x.SimplifyOrElements (nonFalseElems : 'IBoolExpr seq) =
            match Seq.distinct nonFalseElems with
            | Seq.Empty -> FalseExpr
            | Seq.Cons(head, tail) when Seq.isEmpty tail -> head
            | elems -> ctx.MkOrSeq(elems)

        member private x.MkOr ([<ParamArray>] elems : 'IBoolExpr[]) =
        ...
        member private x.MkOr (elems : 'IExpr seq) =
            let elems = elems |> Seq.map (fun m -> ctx.MkEToBE m) |> Seq.toList
        ...
        member private x.MkITE(cond : 'IBoolExpr, thenExpr, elseExpr) : 'IExpr =
            match cond, thenExpr, elseExpr with
            | True, _, _ -> thenExpr
            | False, _, _ -> elseExpr
            | _ when thenExpr = elseExpr -> thenExpr
            | _ when ctx.MkCheckEToBVE(thenExpr) = true && ctx.MkCheckEToBVE elseExpr = true ->
...
// ------------------------------- Encoding: arithmetic simplifications -------------------------------
        ...
        member private x.ExtendIfNeed (f : 'IExpr, y : 'IExpr) isSigned =
            let x = ctx.MkEToBVE f
            let y = ctx.MkEToBVE y
            let difference = int(ctx.SortSize x) - int (ctx.SortSize y)
            if difference = 0 then ctx.MkBVEToE x, ctx.MkBVEToE y
            else
                let extend = if isSigned then ctx.MkSignExt else ctx.MkZeroExt
                if difference > 0 then ctx.MkBVEToE x, ctx.MkBVEToE(extend(uint32 difference, y))
                else ctx.MkBVEToE(extend(uint32 -difference, x)), ctx.MkBVEToE y

        member private this.ChangeSize (x : 'IBitVecExpr) size isSigned =
            let xSize = int (ctx.SortSize x)
        ...
        member private this.ChangeSizeIfNeed (x : 'IExpr, y : 'IExpr as args) typ =
            let x = ctx.MkEToBVE x
            let y = ctx.MkEToBVE y
            let size = numericBitSizeOf typ |> int
            let xSize = int (ctx.SortSize x)
            let ySize = int (ctx.SortSize y)
            if xSize = ySize && xSize = size then args
                ...
                ctx.MkBVEToE x, ctx.MkBVEToE y

        member private x.MkEq(left : 'IExpr, right : 'IExpr) =
            match left, right with
            | _ when left = right -> TrueExpr
            | _ when ctx.MkCheckEToBVE(left) && ctx.MkCheckEToBVE(right) ->
                let l, r = x.ExtendIfNeed(left, right) true
                ctx.MkEq(l, r)
            | _ -> ctx.MkEq(left , right)

        member private x.MkBVSGT(left, right) : 'IBoolExpr =
            match left, right with
            | _ when left = right -> FalseExpr
            | _ -> x.ExtendIfNeed(left, right) true |> ctx.MkBVSGT

        member private x.MkBVUGT(left, right) : 'IBoolExpr =
            match left, right with
            | _ when left = right -> FalseExpr
            | _ -> x.ExtendIfNeed(left, right) false |> ctx.MkBVUGT

        member private x.MkBVSGE(left, right) : 'IBoolExpr =
            match left, right with
            | _ when left = right -> TrueExpr
            | _ ->x.ExtendIfNeed(left, right) true |> ctx.MkBVSGEq

        member private x.MkBVUGE(left, right) : 'IBoolExpr =
            match left, right with
            | _ when left = right -> TrueExpr
            | _ -> x.ExtendIfNeed(left, right) false |> ctx.MkBVUGEq

        member private x.MkBVSLT(left, right ) : 'IBoolExpr =
            match left, right with
            | _ when left = right -> FalseExpr
            | _ -> x.ExtendIfNeed(left, right) true |> ctx.MkBVSLT

        member private x.MkBVULT(left, right) : 'IBoolExpr =
            match left, right with
            | _ when left = right -> FalseExpr
            | _ -> x.ExtendIfNeed(left, right) false |> ctx.MkBVULT

        member private x.MkBVSLE(left, right as operands) : 'IBoolExpr =
            match left, right with
            | _ when left = right -> TrueExpr
            | _ -> x.ExtendIfNeed operands true |> ctx.MkBVSLEq

        member private x.MkBVULE(left, right as operands) : 'IBoolExpr =
            match left, right with
            | _ when left = right -> TrueExpr
            | _ ->
                let left, right = x.ExtendIfNeed operands false
                ctx.MkBVULEq(left, right)

        member private x.MkBVAndT typ operands : 'IExpr =
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVAnd

        member private x.MkBVOrT typ operands : 'IExpr =
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVOr

        member private x.MkBVOr operands : 'IExpr =
            x.ExtendIfNeed operands false |> ctx.MkBVOr

        member private x.MkBVXorT typ operands : 'IExpr =
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVXor

        member private x.MkBVShlT typ operands : 'IExpr =
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVSHL

        member private x.MkBVShl operands : 'IExpr =
            x.ExtendIfNeed operands true |> ctx.MkBVSHL

        member private x.MkBVAShrT typ operands : 'IExpr =
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVASHR

        member private x.MkBVLShrT typ operands : 'IExpr =
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVLSHR

        member private x.MkBVLShr operands : 'IExpr =
            x.ExtendIfNeed operands true |> ctx.MkBVLSHR

        member private x.MkBVAddT typ operands :'IExpr =
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVAdd

        member private x.MkBVAdd operands : 'IExpr =
            x.ExtendIfNeed operands true |> ctx.MkBVAdd

        member private x.MkBVAddNoUnderflow operands : 'IBoolExpr =
            x.ExtendIfNeed operands true |> ctx.MkBVAddNoUnderflow

        member private x.MkBVAddNoOverflow operands : 'IBoolExpr =
            let left, right = x.ExtendIfNeed operands true
            ctx.MkBVAddNoOverflow(left, right, true)

        member private x.MkBVMulT typ operands : 'IExpr =
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVMul

        member private x.MkBVMul operands : 'IExpr =
            x.ExtendIfNeed operands true |> ctx.MkBVMul

        member private x.MkBVMulNoUnderflow operands : 'IBoolExpr =
            x.ExtendIfNeed operands true |> ctx.MkBVMulNoUnderflow

        member private x.MkBVMulNoOverflow operands : 'IBoolExpr =
            let left, right = x.ExtendIfNeed operands true
        ...
        member private x.MkBVSDivT typ operands : 'IExpr =
            assert(isSigned typ)
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVSDiv

        member private x.MkBVUDivT typ operands : 'IExpr =
            assert(isUnsigned typ)
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVUDiv

        member private x.MkBVSRemT typ operands : 'IExpr =
            assert(isSigned typ)
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVSRem

        member private x.MkBVURemT typ operands : 'IExpr =
            assert(isUnsigned typ)
            x.ChangeSizeIfNeed operands typ |> ctx.MkBVURem

        member private x.Max (left : 'IExpr) (right : 'IExpr) =
            assert(ctx.SortSize (ctx.MkEToBVE left) = ctx.SortSize (ctx.MkEToBVE right))
            x.MkITE(x.MkBVSGT(left, right), left, right)

        member private x.Min (left : 'IExpr) (right : 'IExpr) =
            assert(ctx.SortSize (ctx.MkEToBVE left) = ctx.SortSize (ctx.MkEToBVE right))
            x.MkITE(x.MkBVSGT(left, right), right, left)
...
// ------------------------------- Floating point arithmetic simplifications -------------------------------
        ...
        member private x.MkFPAdd(left : 'IExpr, right : 'IExpr) : 'IExpr =
            assert (ctx.GetSort left = ctx.GetSort right)
            ctx.MkFPAdd(RoundNearest, left, right)

        member private x.MkFPMul(left : 'IExpr, right : 'IExpr) : 'IExpr =
            assert (ctx.GetSort left = ctx.GetSort right)
            ctx.MkFPMul(RoundNearest, left, right)

        member private x.MkFPSub(left : 'IExpr, right : 'IExpr) : 'IExpr =
            assert (ctx.GetSort left = ctx.GetSort right)
            ctx.MkFPSub(RoundNearest, left, right)

        member private x.MkFPDiv(left : 'IExpr, right : 'IExpr) : 'IExpr =
            assert (ctx.GetSort left = ctx.GetSort right)
...
// ------------------------------- Encoding: common -------------------------------
        ...
        member public x.EncodeTerm (t : term) : encodingResult<'IExpr, 'IBoolExpr> =
            let getResult () =
                let result =
                    match t.term with
        ...
        member private x.AddEnumAssumptions typ (encodingResult : encodingResult<'IExpr, 'IBoolExpr>) =
            // TODO: support "Flags"
            assert typ.IsEnum
            ...
            let enumAssumptions = x.MkOr (options |> Seq.map (fun m -> ctx.MkBEToE m) |> Seq.toList)
            {expr = expr; assumptions = enumAssumptions :: assumptions}

        member private x.EncodeConcrete (obj : obj) typ : encodingResult<'IExpr, 'IBoolExpr> =
            let expr =
                match typ with
                | Bool ->
                    let res = ctx.MkBool(obj :?> bool)
                    ctx.MkBEToE res
                ...
                | Numeric t when t = typeof<float> ->
                    let sort = x.Type2Sort typ
                    ctx.MkFPDoubleNumeral(obj :?> float, sort)
                | Numeric _ ->
            ...
            encodingResult<'IExpr, 'IBoolExpr>.Create expr

        member private x.EncodeConstant name (source : ISymbolicConstantSource) typ : encodingResult<'IExpr, 'IBoolExpr> =
            match source with
...
// ------------------------------- Encoding: expression -------------------------------
        ...
        member private x.EncodeOperationBV operation args typ =
            ...
            | OperationType.Equal -> x.MakeBinary (ctx.MkBEToE << x.MkEq) args
            | OperationType.NotEqual -> x.MakeBinary (ctx.MkBEToE << x.MkNot << ctx.MkBEToE << x.MkEq) args
            ...
            | OperationType.LogicalNot -> x.MakeUnary (ctx.MkBEToE << x.MkNot) args
            | OperationType.LogicalAnd -> x.MakeOperation (ctx.MkBEToE << x.MkAnd) args
            | OperationType.LogicalOr -> x.MakeOperation (ctx.MkBEToE << x.MkOr) args
            | OperationType.LogicalXor -> x.MakeOperation (ctx.MkBEToE << ctx.MkXor) args
            | OperationType.Greater -> x.MakeBinary (ctx.MkBEToE << x.MkBVSGT) args
            | OperationType.Greater_Un -> x.MakeBinary (ctx.MkBEToE << x.MkBVUGT) args
            | OperationType.GreaterOrEqual -> x.MakeBinary (ctx.MkBEToE << x.MkBVSGE) args
            | OperationType.GreaterOrEqual_Un -> x.MakeBinary (ctx.MkBEToE << x.MkBVUGE) args
            | OperationType.Less -> x.MakeBinary (ctx.MkBEToE << x.MkBVSLT) args
            | OperationType.Less_Un -> x.MakeBinary (ctx.MkBEToE << x.MkBVULT) args
            | OperationType.LessOrEqual -> x.MakeBinary (ctx.MkBEToE << x.MkBVSLE) args
            | OperationType.LessOrEqual_Un -> x.MakeBinary (ctx.MkBEToE << x.MkBVULE) args
        ...
        member private x.EncodeOperationFP operation args =
            match operation with
            | OperationType.Equal -> x.MakeBinary (ctx.MkBEToE << ctx.MkEq) args
            | OperationType.NotEqual -> x.MakeBinary (ctx.MkBEToE << x.MkNot << ctx.MkBEToE << x.MkEq) args
            | OperationType.Greater
            | OperationType.Greater_Un -> x.MakeBinary (ctx.MkBEToE << x.MkFPGt) args
            | OperationType.GreaterOrEqual
            | OperationType.GreaterOrEqual_Un -> x.MakeBinary (ctx.MkBEToE << x.MkFPGEq) args
            | OperationType.Less
            | OperationType.Less_Un -> x.MakeBinary (ctx.MkBEToE << x.MkFPLt) args
            | OperationType.LessOrEqual
            | OperationType.LessOrEqual_Un -> x.MakeBinary (ctx.MkBEToE << x.MkFPLEq) args
        ...
        member private x.ExtractOrExtend (expr : 'IBitVecExpr) size =
            let exprSize = ctx.SortSize expr
            if exprSize = size then expr
            elif exprSize > size then ctx.MkExtract(size - 1u, 0u, expr)
            else ctx.MkSignExt(size - exprSize, expr)

        member private x.ReverseBytes (expr : 'IBitVecExpr) =
            let size = int (ctx.SortSize expr)
            assert(size % 8 = 0)
            let bytes = List.init (size / 8) (fun byte -> ctx.MkExtract(uint ((byte + 1) * 8) - 1u, uint (byte * 8), expr))
            List.reduce (fun (x : 'IBitVecExpr) (y : 'IBitVecExpr) -> ctx.MkConcat(x, y)) bytes

        member private x.ComputeSliceBounds assumptions cuts termSortSize =
            assert(termSortSize % 8u = 0u && termSortSize > 0u)
            let zero = ctx.MkBVInt(0, termSortSize) |> ctx.MkBVEToE
            let sizeExpr = ctx.MkBV(termSortSize / 8u, termSortSize) |> ctx.MkBVEToE
            let addBounds (startByte, endByte, pos) (assumptions, startExpr, sizeExpr, position) =
                ...
                let startByte = x.ExtractOrExtend (ctx.MkEToBVE(startByte.expr)) termSortSize |> ctx.MkBVEToE
                let endByte = x.ExtractOrExtend (ctx.MkEToBVE(endByte.expr)) termSortSize |> ctx.MkBVEToE
                let pos = x.ExtractOrExtend (ctx.MkEToBVE(pos.expr)) termSortSize |> ctx.MkBVEToE
                let startByte = x.MkBVSub(startByte, position) |> ctx.MkBVEToE
                let endByte = x.MkBVSub(endByte, position) |> ctx.MkBVEToE
                let left = x.Max startByte zero
                let right = x.Min endByte sizeExpr
                let sliceSize = x.MkBVSub(right, left) |> ctx.MkBVEToE
        ...
        member private x.EncodeBoolBitVec (b : 'IBoolExpr) =
            ...
            ctx.MkEToBVE(x.MkITE(b, trueBitVec, falseBitVec))
        ...
        member private x.CreateCombineResult result typ size =
            if isReal typ then
                ...
                let res = ctx.MkFP(signExpr, expExpr, sigExpr)
                ctx.MkFPEToE res
            else ctx.MkBVEToE result

        member private x.EncodeCombine slices typ =
            let size = x.SizeOfBV typ
            let res = ctx.MkBVInt(0, size)
            let window = ctx.SortSize res
            let windowExpr = ctx.MkEToBVE(ctx.MkNumeral(window, x.Type2Sort(Types.IndexType)))
            let addOneSlice (res, assumptions) slice =
                ...
                let t =
                    ...
                    | _ when ctx.MkCheckEToBVE term.expr -> ctx.MkEToBVE term.expr
                    | _ when ctx.MkCheckEToBE term.expr -> x.EncodeBoolBitVec (ctx.MkEToBE term.expr)
                    | _ when ctx.MkCheckEToFPE term.expr -> ctx.MkFPToIEEEBV(ctx.MkEToFPE term.expr)
                    | _ -> internalfail $"EncodeCombine: unexpected slice term {term}"
                let assumptions = assumptions @ term.assumptions
                let termSize = ctx.SortSize t
                let sizeExpr = ctx.MkBV(termSize, termSize) |> ctx.MkBVEToE
                let assumptions, lByte, rByte, posByte = x.ComputeSliceBounds assumptions cuts termSize
                let lByte = x.ExtractOrExtend (ctx.MkEToBVE lByte) termSize |> ctx.MkBVEToE
                let rByte = x.ExtractOrExtend (ctx.MkEToBVE rByte) termSize |> ctx.MkBVEToE
                let posByte = x.ExtractOrExtend (ctx.MkEToBVE posByte) termSize |> ctx.MkBVEToE
                let lBit = x.MkBVMul(lByte, ctx.MkBVEToE(ctx.MkBVInt(8, termSize)))
                let rBit = x.MkBVMul(rByte, ctx.MkBVEToE(ctx.MkBVInt(8, termSize)))
                let posBit = x.MkBVMul(posByte, ctx.MkBVEToE(ctx.MkBVInt(8, termSize))) |> ctx.MkEToBVE
                let sliceSize = x.MkBVSub(rBit, lBit) |> ctx.MkBVEToE
                let zero = ctx.MkBVInt(0, termSize) |> ctx.MkBVEToE
                let intersects = x.MkBVSGT(sliceSize, zero)
                let term = x.ReverseBytes t |> ctx.MkBVEToE
                let left = x.ExtractOrExtend (ctx.MkEToBVE lBit) termSize |> ctx.MkBVEToE
                let term = x.MkBVShl(term, left)
                let cutRight = x.ExtractOrExtend (x.MkBVSub(sizeExpr, rBit)) termSize |> ctx.MkBVEToE
                let term = x.MkBVLShr(term, x.MkBVAdd(left, cutRight)) |> ctx.MkEToBVE
                let term =
                    if termSize > window then ctx.MkExtract(window - 1u, 0u, term)
                    else ctx.MkZeroExt(window - termSize, term)
                let changedTermSize = ctx.SortSize term
                let w = x.ExtractOrExtend windowExpr changedTermSize |> ctx.MkBVEToE
                let pos = x.ExtractOrExtend posBit changedTermSize |> ctx.MkBVEToE
                let sliceSize = x.ExtractOrExtend (ctx.MkEToBVE sliceSize) changedTermSize |> ctx.MkBVEToE
                let shift = x.MkBVSub(w, x.MkBVAdd(pos, sliceSize)) |> ctx.MkBVEToE
                let part = x.MkBVShl(ctx.MkBVEToE term, shift)
                let res = ctx.MkEToBVE(x.MkITE(intersects,x.MkBVOr(ctx.MkBVEToE res, part), ctx.MkBVEToE(res)))
                res, assumptions
        ...
        member private x.EncodeApplication sf typ (args : 'IExpr array) =
            let argsSort = args |> Array.map (fun arg -> ctx.GetSort arg)
        ...
        member private x.EncodeExpression term op args typ =
            encodingCache.Get(term, fun () ->
                ...
                | Cast(Numeric t1, Numeric t2) when isReal t1 && isReal t2 ->
                    ...
                    let res = ctx.MkFPEToE(ctx.MkFPToFP(RoundNearest,ctx.MkEToFPE arg.expr, sort))
                ...
                | Cast(Numeric t1, Numeric t2) when isReal t1 ->
                    ...
                    let res = ctx.MkFPToBV(RoundNearest, ctx.MkEToFPE(arg.expr), size, sign)
                    {expr = ctx.MkBVEToE res; assumptions = arg.assumptions}
                | Cast(Numeric t1, Numeric t2) when isReal t2 ->
                    let arg = x.EncodeTerm (List.head args)
                    let argExpr = ctx.MkEToBVE arg.expr
                    let sort = x.Type2Sort t2
                    let sign = isSigned t1
                    let res = ctx.MkFPToFP(RoundNearest, argExpr, sort, sign)
                    {expr = ctx.MkFPEToE res; assumptions = arg.assumptions}
                | Cast(Numeric t1, Numeric t2) when isLessForNumericTypes t1 t2 ->
                    ...
                    {expr = ctx.MkBVEToE(extend(difference, ctx.MkEToBVE expr.expr)); assumptions = expr.assumptions}
                | Cast(Numeric t1, Numeric t2) when isLessForNumericTypes t2 t1 ->
                    let expr = x.EncodeTerm (List.head args)
                    let from = numericBitSizeOf t2 - 1u
                    {expr =  ctx.MkBVEToE(ctx.MkExtract(from, 0u, ctx.MkEToBVE expr.expr)); assumptions = expr.assumptions}
                | Cast(Numeric t1, Numeric t2) when numericBitSizeOf t1 = numericBitSizeOf t2 ->
                    x.EncodeTerm (List.head args)
                | Cast(Bool, Numeric t) ->
                    let arg = x.EncodeTerm (List.head args)
                    let expr = ctx.MkEToBE arg.expr
                ...
                | Cast(Numeric t, Bool) ->
                    let arg = x.EncodeTerm (List.head args)
                    let expr = ctx.MkEToBVE arg.expr
                    let sort = x.Type2Sort(t)
                    let converted = x.MkEq(ctx.MkBVEToE expr, ctx.MkNumeral(0, sort)) |> ctx.MkBEToE |> x.MkNot |> ctx.MkBEToE
                    {expr = converted; assumptions = arg.assumptions}
        ...
        member private this.MakeUnary
                (constructor : 'IExpr -> 'IExpr)
                (args : term list) : encodingResult<'IExpr, 'IBoolExpr> =
            match args with
            | [x] ->
        ...
        member private this.MakeBinary
                (constructor : 'IExpr * 'IExpr -> 'IExpr)
                (args : term list) : encodingResult<'IExpr, 'IBoolExpr> =
            match args with
            | [x; y] ->
        ...
        member private this.MakeOperation
                (constructor : 'IExpr [] -> 'IExpr)
                (args : term list) : encodingResult<'IExpr, 'IBoolExpr> =
            let assumptions, expressions = this.EncodeTerms args
            {expr = constructor expressions; assumptions = assumptions}

        member internal x.EncodeTerms (ts : term seq) : 'IBoolExpr list * 'IExpr array =
...
// ------------------------------- Encoding: memory reading -------------------------------
        ...
        member private x.GetRegionConstant (name : string) sort (path : path) (regionSort : regionSort) =
            ...
            let mkConst () = ctx.MkEToAE(ctx.MkConst(constName, sort))
        ...
        member private x.MemoryReading specializeWithKey keysAreMatch encodeKey inst (path : path) readKey mo =
            ...
            encodingResult<'IExpr, 'IBoolExpr>.Create(expr, assumptions)
        ...
        member private x.HeapReading key mo typ source path name =
            ...
            let keysAreMatch read update updateRegion =
                ...
                encodeAssumptions, ctx.MkEToBE(matchConditionEncoded)
        ...
        member private x.GenerateInstAssumptions (inst : 'IExpr) typ =
            if typ = addressType then
                // In case of symbolic address instantiation, adding assumption "inst < 0"
                let zero = ctx.MkNumeral(0, ctx.GetSort inst)
                let belowZero = x.MkBVSLE(inst, zero)
        ...
        member private x.GenerateLengthAssumptions encodingResult  =
            ...
            let lengthIsNonNegative = x.MkBVSGE(expr, ctx.MkBVEToE(ctx.MkBVInt(0, ctx.SortSize(ctx.MkEToBVE expr))))
            let assumptions = lengthIsNonNegative :: assumptions
            let assumptions =
                if maxBufferSize < 0 then assumptions
                else x.MkBVSLE(expr, ctx.MkBVInt(maxBufferSize, ctx.SortSize(ctx.MkEToBVE expr)) |> ctx.MkBVEToE) :: assumptions
        ...
        member private x.ArrayReading specialize keysAreMatch encodeKey hasDefaultValue indices key mo typ source path name =
            ...
            let inst (k : 'IExpr[]) =
                ...
                if hasDefaultValue then x.DefaultValue valueSort, List.empty
                    ...
                    let sort = ctx.MkRangeArraySort(domainSort, valueSort)
                    let array = x.GetRegionConstant name sort path regionSort
                    let expr = ctx.MkRangeSelect(array, k)
                    expr, x.GenerateInstAssumptions expr typ
            let res = x.MemoryReading specialize keysAreMatch encodeKey inst path key mo
        ...
        member private x.ArrayIndexReading key hasDefaultValue mo typ source path name =
            ...
            let keysAreMatch read update updateRegion =
                ...
                assumptions, ctx.MkEToBE matchConditionEncoded
            let indices =
                match key with
        ...
        member private x.StructReading (structSource : ISymbolicConstantSource) (field : fieldId) typ (path : path) name =
            ...
            | _ when field.declaringType = typeof<decimal> && field.name = "_flags" ->
                let expr = ctx.MkEToBVE res.expr
                let lowerWord = ctx.MkExtract(15u, 0u, expr)
                let lowerIsZero = x.MkEq(ctx.MkBVEToE lowerWord, ctx.MkBVEToE(ctx.MkBVInt(0, ctx.SortSize lowerWord)))
                let exp = ctx.MkExtract(23u, 16u, expr)
                let expSize = ctx.SortSize exp
                let leftBound = x.MkBVUGE(ctx.MkBVEToE exp, ctx.MkBVEToE(ctx.MkBVInt(0, expSize)))
                let rightBound = x.MkBVULE(ctx.MkBVEToE exp, ctx.MkBVEToE(ctx.MkBVInt(28, expSize)))
                let expInBound = x.MkAnd(leftBound, rightBound)
                let upper = ctx.MkExtract(30u, 24u, expr)
                let upperIsZero = x.MkEq(ctx.MkBVEToE upper, ctx.MkBVEToE(ctx.MkBVInt(0, ctx.SortSize upper)))
                { res with assumptions = lowerIsZero::expInBound::upperIsZero::res.assumptions }
            | _ -> res

        member private x.EncodeMemoryAccessConstant name (source : ISymbolicConstantSource) (path : path) typ : encodingResult<'IExpr, 'IBoolExpr> =
            match source with
    ...
    // ------------------------------- Decoding -------------------------------
        ...
        member private x.DecodeExpr op t (expr : 'IExpr) =
            // TODO: bug -- decoding arguments with type of expression
            Expression (Operator op) (ctx.GetArgs expr |> Seq.map (x.Decode t) |> List.ofSeq) t

        member private x.DecodeBoolExpr op (expr : 'IExpr) =
            x.DecodeExpr op typeof<bool> expr

        member private x.GetTypeOfBV (bv : 'IBitVecExpr) =
            let size = ctx.SortSize bv
            if size = 32u then typeof<int32>
            elif size = 64u then typeof<int64>
            elif size = 8u then typeof<int8>
            elif size = 16u then typeof<int16>
            else __unreachable__()

        member private x.DecodeConcreteHeapAddress (expr : 'IExpr) : vectorTime =
            let addresses = encodingCache.heapAddresses
            let result = ref vectorTime.Empty
            let existing = addresses.TryGetValue(expr, result)
            match expr with
            | _ when ctx.MkCheckEToBVNum expr && ctx.Int64 (ctx.MkEToBVNum expr) = 0L -> VectorTime.zero
        ...
        member private x.DecodeSymbolicTypeAddress (expr : 'IExpr) =
            let result = ref typeof<Void>
            if encodingCache.staticKeys.TryGetValue(expr, result) then result.Value
            else __notImplemented__()

        member private x.DecodeMemoryKey (reg : regionSort) (exprs : 'IExpr array) =
            match reg with
            | HeapFieldSort field ->
        ...
        member private x.DecodeBv t (bv : 'IBitVecNum) =
            ...
            | 64u -> Concrete (convert (uint64(ctx.BigInteger bv)) t) t
            | 16u -> Concrete (convert (ctx.Int bv) t) t
            | 8u  -> Concrete (convert (ctx.Int bv) t) t
            | _ -> __notImplemented__()

        member private x.DecodeFPNum t (fp : 'IFPNum) =
            let typeIsSingle = t = typeof<Single>
            let typeIsDouble = t = typeof<Double>
            let sizeExp = int (ctx.FPEBits fp)
            let exprIsSingle = sizeExp = 8
            let exprIsDouble = sizeExp = 11
            assert(exprIsSingle && typeIsSingle || exprIsDouble && typeIsDouble)
            let number : obj =
                match fp with
                | _ when ctx.IsNaN fp && exprIsSingle -> Single.NaN
                | _ when ctx.IsNaN fp && exprIsDouble -> Double.NaN
                | _ when ctx.IsFPPlusInfinity fp && exprIsSingle -> Single.PositiveInfinity
                | _ when ctx.IsFPPlusInfinity fp && exprIsDouble -> Double.PositiveInfinity
                | _ when ctx.IsFPMinusInfinity fp && exprIsSingle -> Single.NegativeInfinity
                | _ when ctx.IsFPMinusInfinity fp && exprIsDouble -> Double.NegativeInfinity
                | _ ->
                    let sizeVal = if exprIsSingle then 23 else 52
                    let sign = if ctx.Sign fp then "1" else "0"
                    let exp = ctx.ExponentInt64 fp
                    let value = int64 (ctx.SignificandUInt64 fp)
        ...
        member public x.Decode t (expr : 'IExpr) =
            match expr with
            | _ when ctx.MkCheckEToBVNum expr && Types.IsNumeric t -> x.DecodeBv t (ctx.MkEToBVNum expr)
            ...
            | _ ->
                if encodingCache.e2t.ContainsKey(expr) then encodingCache.e2t[expr]
                elif ctx.IsTrue expr then True()
                elif ctx.IsFalse expr then False()
                elif ctx.IsNot expr then x.DecodeBoolExpr OperationType.LogicalNot expr
                elif ctx.IsAnd expr then x.DecodeBoolExpr OperationType.LogicalAnd expr
                elif ctx.IsOr expr then x.DecodeBoolExpr OperationType.LogicalOr expr
                elif ctx.IsEq expr then x.DecodeBoolExpr OperationType.Equal expr
                elif ctx.IsBVSGT expr then x.DecodeBoolExpr OperationType.Greater expr
                elif ctx.IsBVUGT expr then x.DecodeBoolExpr OperationType.Greater_Un expr
                elif ctx.IsBVSGE expr then x.DecodeBoolExpr OperationType.GreaterOrEqual expr
                elif ctx.IsBVUGE expr then x.DecodeBoolExpr OperationType.GreaterOrEqual_Un expr
                elif ctx.IsBVSLT expr then x.DecodeBoolExpr OperationType.Less expr
                elif ctx.IsBVULT expr then x.DecodeBoolExpr OperationType.Less_Un expr
                elif ctx.IsBVSLE expr then x.DecodeBoolExpr OperationType.LessOrEqual expr
                elif ctx.IsBVULE expr then x.DecodeBoolExpr OperationType.LessOrEqual_Un expr
                elif ctx.IsFPGT expr then x.DecodeBoolExpr OperationType.Greater expr
                elif ctx.IsFPGE expr then x.DecodeBoolExpr OperationType.GreaterOrEqual expr
                elif ctx.IsFPLT expr then x.DecodeBoolExpr OperationType.Less expr
                elif ctx.IsFPLE expr then x.DecodeBoolExpr OperationType.LessOrEqual expr
                else __notImplemented__()
        ...
        member private x.WriteToPrimitiveModel (subst : IDictionary<ISymbolicConstantSource, term ref>) (m : 'IModel) (path : path) source expr typ constant =
            let refinedExpr = ctx.Eval(m, expr, false)
        ...
        member private x.FillRegion state (m : 'IModel) (regionSort : regionSort) constantValue =
            match regionSort with
            | HeapFieldSort field ->
                ...
                for expressions in getDefaultValues regionSort do
                    ...
                    let refinedAddress = ctx.Eval(m, expressions[0], false)
            ...
            | ArrayLengthSort arrayType ->
                ...
                for expressions in getDefaultValues regionSort do
                    ...
                    let refinedAddress = ctx.Eval(m, expressions[0], false)
                    let address = x.DecodeConcreteHeapAddress refinedAddress |> ConcreteHeapAddress
                    let index = ctx.Eval(m, expressions[1], false) |> x.Decode typeof<int>
            ...
            | StackBufferSort key ->
                ...
                for expressions in getDefaultValues regionSort do
                    assert(Array.length expressions = 1)
                    let index = ctx.Eval(m, expressions[0], false) |> x.Decode typeof<int8>
            ...
            try
                ...
                for KeyValue(key, value) in encodingCache.t2e do
                    ...
                    | {term = Constant(_, Path.Path(path, StackReading(key)), t)} as constant ->
                        let refinedExpr = ctx.Eval(m, value.expr, false)
                ...
                for KeyValue((region, path), constant) in encodingCache.regionConstants do
                    ...
                    let rec parseArray (arr : 'IExpr) =
                        if ctx.IsConstantArray arr then
                            assert(ctx.GetArgs(arr).Length = 1)
                            let constantValue = x.Decode typeOfLocation (ctx.GetArgs(arr)[0])
                            x.WriteDictOfValueTypes defaultValues region path typeOfRegion constantValue
                        elif ctx.IsDefaultArray arr then
                            assert(ctx.GetArgs(arr).Length = 1)
                        elif ctx.IsStore arr then
                            assert(ctx.GetArgs(arr).Length >= 3)
                            parseArray (ctx.GetArgs(arr)[0])
                            let address = x.DecodeMemoryKey region (ctx.GetArgs(arr)[1..ctx.GetArgs(arr).Length - 2])
                            let value = Array.last (ctx.GetArgs arr) |> x.Decode typeOfLocation
                            if path.IsEmpty then
    ...
    type CommonSolver(ctx : SolverBuilder<'IExpr, 'IBoolExpr, 'IBitVecExpr, 'IFPExpr, 'IArrayExpr, 'IBitVecNum, 'IFPNum, 'IFuncDecl,
        ...
        interface ISolver with
            member x.CheckSat (q : term) : smtResult =
                ...
                try
                    try
                        ...
                        let result = ctx.MkSCheck(solver, assumptions)
                        match result with
                        | IStatus.SATISFIABLE ->
                             Logger.trace "SATISFIABLE"
                             let SolverModel = ctx.MkSModel(solver)
                             let model = ctx.MkModel SolverModel
                             SmtSat { mdl = model }
                        | IStatus.UNSATISFIABLE ->
                             Logger.trace "UNSATISFIABLE"
                             SmtUnsat { core = Array.empty (*optCtx.UnsatCore |> Array.map (builder.Decode Bool)*) }
                        | IStatus.UNKNOWN ->
                            Logger.error $"Solver returned Status.UNKNOWN. Reason: {ctx.GetReasonUnknown solver}\n\
                                Expression: {assumptions |> Seq.map (fun a -> a.ToString()) |> Seq.toList}"

                            Logger.trace "UNKNOWN"
                            SmtUnknown (ctx.GetReasonUnknown solver)
                        | _ -> __unreachable__()
                    with
            ...
            member x.Assert (fml : term) =
                ...
                let encoded = List.fold (fun acc x -> ctx.MkAnd(acc, x)) (ctx.MkEToBE encoded.expr) encoded.assumptions
                ctx.MkSAssert(solver, encoded)
